...
[오프라인 / CI]
  ...
  GitHub Actions ─▶ 데일리 별자리 운세 생성 스크립트 ─▶ S3 직접 업로드
  (2026-07 기준 cron 실행 실패 중 — requirements.txt 부재로 pip 캐시 스텝에서 매번 실패)
```
...
| `/career` | 커리어 운 (관성 진입 가중치) |
| `/compatibility` | 이상형 역산 (원하는 사주 조건 → 생년월일 후보 역추적) |
| `/couple` | 커플 궁합 스코어링 |
| `/news` | 점검 안내 화면 (뉴스 검색 API 회귀로 임시 대체 중) |
| `/blog` | 블로그 목록·상세 (2026-07-09 `/blog/admin` 발행 UI는 비밀번호 노출 문제 + MBTI 비공유 방침으로 제거) |
