...
### 전체 디렉토리 트리 (depth 3, `node_modules`/`.git`/`.next` 제외)

```
.
├── frontend-next/         Next.js 16 App Router 정적 export 프론트엔드 (S3+CloudFront 서빙)
├── scripts/                사주 캐시 생성, 블로그 자동화, Lambda, 인프라 스크립트 (Python 위주)
...
`src/`(앱 코드), `public/`(정적 자산·`saju-cache/`), `out/`(정적 export 빌드 산출물), `scripts/`(`deploy.sh`, `split-characters.py`), `.phase/`(마이그레이션 기록), `package.json`, `next.config.ts`, `CLAUDE.md`/`AGENTS.md`(프론트 전용 가이드).

> **주의**: `deploy.sh`가 저장소에 **두 벌** 존재한다 — 루트 `scripts/deploy.sh`(S3 sync+CloudFront invalidation 실제 배포 스크립트)와 `frontend-next/scripts/deploy.sh`. 두 파일의 관계(둘 다 쓰이는지, 하나가 레거시인지)는 이번 조사에서 내용까지 diff하지 않아 **추정** — 다음 조사에서 확인 필요.

### `frontend-next/src/` (FSD 스타일 레이어, depth 3)
...
> `features/` 하위에 `chatbot`, `characters`, `concern-card`, `iching`, `points`가 실제로 존재하는데, 이는 `frontend-next/CLAUDE.md`가 기술하는 "3개만 있음(`fortune`, `couple-match`, `ideal-match`)"과 다르다 — **문서 드리프트**. (§4-3 근거)

### 진입점 (Entry points)

- `frontend-next/src/app/layout.tsx` — 루트 레이아웃, `Providers`/`GoogleAnalytics`/`ClarityAnalytics`/`TopNav`/JSON-LD 조립
- `frontend-next/src/app/page.tsx` — 랜딩 페이지(`'use client'`)
- `scripts/deploy.sh` — 배포 실행 스크립트
- `frontend-next/scripts/split-characters.py` — 캐릭터 이미지 분할 유틸
- `scripts/lambda/chat-bedrock/handler.py` — Bedrock 챗봇 Lambda 핸들러 (Function URL)
- `scripts/lambda/chat-bedrock/local_server.py` — 로컬 테스트 서버
- `scripts/lambda/blog-publish/handler.py` — 블로그 발행 Lambda (Function URL, `AuthType=NONE`)
- `scripts/generate_parallel.py`, `generate_parallel_en.py`, `generate_saju_cache.py`, `generate_today_parts.py`, `generate_today_parts_en.py`, `generate_today_variants.py`, `generate_tone_variants.py`, `generate_blog_daily_zodiac.py` — Bedrock 호출 CLI 스크립트(`argparse`)
- `scripts/upload_blog_post.py`, `scripts/merge_chongun_en.py`, `scripts/clean_today_cache.py` — 보조 CLI
- `scripts/cloudfront/apply-security-headers.sh`, `scripts/cloudfront/rewrite-subdir-index.js` — 인프라 스크립트
...
### `frontend-next/package.json` 의존성 (`frontend-next/package.json:11-30`)
...
| 패키지 | 검증 | 결과 |
|---|---|---|
| `aws-amplify` | `grep -rn "aws-amplify\|amplify" src` | 0건. `frontend-next/CLAUDE.md:144`에 "현재 사용처 없음" 기재와 **일치** |
| `@aws-sdk/eventstream-codec` | 전체 검색 | 0건. `frontend-next/CLAUDE.md:145` 기재와 **일치** |
| `@aws-sdk/util-utf8` | 전체 검색 | 0건. `frontend-next/CLAUDE.md:145` 기재와 **일치** |
| `aromanize` | `grep -rl "aromanize" src` | `frontend-next/src/shared/types/aromanize.d.ts:1`에 앰비언트 타입 선언만 존재. `from ['"]aromanize['"]` 실제 import는 **0건**. 타입 선언만 있고 런타임 사용처 없음 (추정: 과거 사용 후 제거되었거나 도입 예정이었으나 미완) |
...
`requirements.txt` 등 의존성 명세 파일이 저장소 전체에 **없음** (`find . -iname 'requirements*.txt'` 결과 0건). `scripts/*.py`는 `boto3` 등을 import하지만 버전이 어디에도 고정되어 있지 않다 (추정: 로컬/CI에 수동 설치).
...
- 배치 캐시 생성(`scripts/generate_*.py` 8개 전부) — 동일한 Application Inference Profile ARN 사용: `arn:aws:bedrock:us-east-1:887078546492:application-inference-profile/cybevkpbbz32`, region `us-east-1`. 근거: `scripts/generate_parallel.py:14`, `generate_parallel_en.py:20`, `generate_saju_cache.py:17`, `generate_today_parts.py:17`, `generate_today_parts_en.py:19`, `generate_today_variants.py:15`, `generate_tone_variants.py:17`, `generate_blog_daily_zodiac.py:25`
- 런타임 챗봇(`scripts/lambda/chat-bedrock/handler.py:37,39`) — `bedrock-runtime` 클라이언트 직접 사용, 모델 ID는 env `BEDROCK_MODEL_ID`(기본값 `anthropic.claude-haiku-4-5`)

### S3

- `scripts/deploy.sh:19` — `SAJU_BUCKET="saju-oracle-frontend-887078546492"` (ap-northeast-2)
- `scripts/lambda/blog-publish/handler.py:37-40`, `scripts/generate_blog_daily_zodiac.py:29-31`, `scripts/upload_blog_post.py:23-25` — 동일한 `S3_TARGETS` 배열로 `sedaily-mbti-frontend-dev`(us-east-1) + `saju-oracle-frontend-887078546492`(ap-northeast-2) **두 버킷에 이중 업로드**

### CloudFront

- `scripts/deploy.sh:20,55-56` — `SAJU_DIST="E2ZDGPQU5JXQKC"`
...
- `scripts/cloudfront/apply-security-headers.sh:20` — `DISTRIBUTION_ID="E2ZDGPQU5JXQKC"`
- `scripts/cloudfront/rewrite-subdir-index.js` — CloudFront Function, 배포 방식은 스크립트 밖(콘솔/CLI 수동, 추정)

### Lambda / API Gateway / Function URL

- `frontend-next/src/app/blog/admin/page.tsx:15` — 블로그 발행 Function URL 프런트에 하드코딩: `https://2ranuwiguucfnrw7ks5jkjhami0zupuu.lambda-url.us-east-1.on.aws/`
- `frontend-next/src/shared/config/api.ts:7` — MBTI 백엔드 API Gateway: `https://chzwwtjtgk.execute-api.us-east-1.amazonaws.com/dev` (이 레포에는 해당 Lambda 소스 없음, `AI-CUSTOMIZED-MBTI` 별도 레포 관리)
- `scripts/lambda/chat-bedrock/handler.py:51` — 같은 API를 서버 간 프록시로 재호출(`SEARCH_API_URL`)

**`scripts/lambda/` 함수별**
1. `chat-bedrock/handler.py` — 사주 챗봇 실시간 LLM 프록시. Bedrock + 조건부 DynamoDB(사용량 상한) + 외부 HTTP(뉴스 프록시). 트리거: **Function URL**(`auth-type NONE`, `scripts/lambda/chat-bedrock/README.md:49-51`)
2. `blog-publish/handler.py` — 블로그 발행. S3 put_object(2버킷) + CloudFront invalidation. 트리거: **Function URL**(`auth-type NONE`). 비밀번호는 env `ADMIN_PASS`와 body 단순 비교(`handler.py:85`)

### DynamoDB

`scripts/lambda/chat-bedrock/handler.py:46-89` — `USAGE_TABLE`(env, 기본 미설정=비활성)에 `update_item`으로 일일 호출 카운터 관리(킬스위치), TTL 172800초(2일)

### Bedrock 스크립트 요약 (`scripts/`)

- `generate_saju_cache.py` — 사주 원국 해석 배치 생성 (추정: 로컬 캐시만, S3 업로드 없음)
...
**프런트** (`scripts/deploy.sh:1-69`):
1. `npm run build`(`:39`, `--no-build`로 스킵 가능) → `frontend-next/out/`
2. `out/` → `out-saju/` 복사(`:46-48`)
3. `aws s3 sync out-saju/ s3://saju-oracle-frontend-887078546492/ --region ap-northeast-2 --delete`(`:52-53`)
4. `aws cloudfront create-invalidation --distribution-id E2ZDGPQU5JXQKC --paths "/*"`(`:54-59`)

**블로그 Lambda**: 자동화 스크립트 없음. `scripts/lambda/chat-bedrock/README.md:39-52`에 CLI 예시만 문서화(`zip` → `aws lambda create-function` → `create-function-url-config`). `blog-publish`는 코드 변경 시 `fn.zip` 수동 재패킹 + `update-function-code` 필요(`CLAUDE.md` 작업 규칙 5) — 이를 자동화하는 `.sh`는 레포에 없음(`policy.json`/`trust-policy.json`/`cors.json`은 파일로 준비되어 있으나 적용은 수동 CLI로 추정).
...
### 4-0. 보안 — 평문 비밀번호 커밋 (가장 심각)

**`frontend-next/src/app/blog/admin/page.tsx:12`** — `const ADMIN_PASS = 'sedaily2024!';`

블로그 어드민 로그인 비밀번호가 클라이언트 소스에 평문으로 커밋되어 있고, 정적 export 특성상 **빌드된 JS 번들에 그대로 노출**된다. 브라우저 개발자도구로 누구나 확인 가능.

### 4-1. 300줄 넘는 파일 (상위 20개)
...
### 4-3. 순환 참조 / feature 간 직접 import 위반
...
→ `couple-match`가 `fortune`과 `ideal-match` 둘 다 직접 참조하는 구조(순환 위험). `frontend-next/eslint.config.mjs:34-53`에 `boundaries/element-types` 규칙이 있으나 **severity가 `"warn"`**(`:29`)이라 빌드를 막지 못한다.
...
또한 `features/`에 `frontend-next/CLAUDE.md`가 언급하지 않는 `chatbot`, `characters`, `concern-card`, `iching`, `points`가 실제로 존재한다 — **문서 드리프트** (신규 기능이 계속 추가되는데 CLAUDE.md가 갱신되지 않은 것으로 추정).

### 4-4. 하드코딩된 값

**시크릿**: §4-0 참조 (`app/blog/admin/page.tsx:12`).
...
AWS 버킷명/CloudFront ID는 `frontend-next/src/` 안에서는 검색되지 않는다(스크립트 쪽에만 존재, §3 참조).
...
`find frontend-next/src -iname "*.test.ts*" -o -iname "*.spec.ts*" -o -type d -iname "__tests__"` → **없음, 확인됨**. `frontend-next/CLAUDE.md`의 "테스트: 없음" 기술과 실제 상태 일치.
...
**ESLint boundaries 설정** — `frontend-next/eslint.config.mjs:20-53`에 `eslint-plugin-boundaries`가 있으나:
...
| 영역 | 상태 |
|---|---|
| IaC | 없음. 배포는 `scripts/deploy.sh` bash 스크립트 + 하드코딩된 리소스 ID |
| 보안 | `app/blog/admin/page.tsx:12`에 평문 비밀번호 커밋·번들 노출 — 가장 시급 |
| 테스트 | 없음 (문서와 일치, 신규 사실 아님) |
| 아키텍처 경계 | `features/fortune`을 중심으로 feature 간 직접 import 다수, ESLint 규칙은 warn이라 강제되지 않음 |
| 문서 드리프트 | `frontend-next/CLAUDE.md`가 "feature 3개"라 기술하지만 실제 8개 존재 |
| 대형 파일 | `engine-chaeun.ts`(1768줄), `FortuneResult.tsx`(1302줄), `engine.ts`(1259줄) 등 `features/fortune`에 집중 |
