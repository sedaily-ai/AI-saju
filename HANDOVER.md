...
| 배포 명령 | `cd frontend && npm run deploy` |
...
├── scripts/
│   ├── deploy.sh              (미사용 — frontend/scripts/frontend/deploy.sh 로 대체됨)
...
레포 추적 파일 **2,368개** 중 약 **94%가 `scripts/backend/saju-cache-local/`** 의 JSON. 실제 소스 코드는 200개 미만. 자세한 내역은 [§11](#11-사주-해석-캐시-생성) 참고.
...
| 경로 | 파일 | 주요 컴포넌트/엔진 | 역할 |
|------|------|----------------------|------|
| `/` | [app/page.tsx](frontend/src/app/page.tsx) | — | 랜딩 (Saju 소개, Pitch, What we do, Method, For whom, CTA) |
| `/saju/` | [app/saju/page.tsx](frontend/src/app/saju/page.tsx) | `FortuneTab` | 사주 입력 + 원국·총운·오늘의 운세 결과 |
| `/chaeun/` | [app/chaeun/page.tsx](frontend/src/app/chaeun/page.tsx) | `engine-chaeun.ts`, `WealthNewsSection` | 재운 흐름 (대운·세운·월운 + 12개월 타임라인 + 경제뉴스) |
| `/career/` | [app/career/page.tsx](frontend/src/app/career/page.tsx) | `engine-chaeun.ts` (커리어 재해석), `CareerNewsSection` | 커리어 운 (관성·식상 + 12개월 + 경제뉴스) |
| `/compatibility/` | [app/compatibility/page.tsx](frontend/src/app/compatibility/page.tsx) | `ideal-match/matchEngine.ts` | 이상형 사주 역산 (상대 없이) |
| `/couple/` | [app/couple/page.tsx](frontend/src/app/couple/page.tsx) | `couple-match/coupleEngine.ts` | 두 사람 궁합 스코어링 |
| `/news/` | [app/news/page.tsx](frontend/src/app/news/page.tsx) | `features/fortune` + API 호출 | 키워드 기반 경제뉴스 탐색 |
| `/blog/` | [app/blog/page.tsx](frontend/src/app/blog/page.tsx) | fetch `/blog-content/index.json` | 블로그 목록 + 상세 (동적 렌더, slug param) |
| `/blog/admin/` | [app/blog/admin/page.tsx](frontend/src/app/blog/admin/page.tsx) | Lambda Function URL POST | 블로그 발행 (비밀번호 보호) |
| `/about/` | [app/about/layout.tsx + page.tsx](frontend/src/app/about/) | — | `/` 로 meta-refresh + JS 리다이렉트 (레거시 URL 유지) |

### 4.1 라우트 네비 (FeatureTabs)

상단 고정 탭 네비. [src/widgets/FeatureTabs.tsx](frontend/src/widgets/FeatureTabs.tsx):
...
### 5.1 핵심 함수 — [engine.ts](frontend/src/features/fortune/lib/engine.ts) (1,259 줄)
...
### 5.2 재운 엔진 — [engine-chaeun.ts](frontend/src/features/fortune/lib/engine-chaeun.ts) (1,768 줄)
...
### 5.3 커플 궁합 — [coupleEngine.ts](frontend/src/features/couple-match/lib/coupleEngine.ts) (201 줄)
...
### 5.4 이상형 역산 — [matchEngine.ts](frontend/src/features/ideal-match/lib/matchEngine.ts) (265 줄)
...
- [cheongan_db.json](frontend/src/features/fortune/lib/cheongan_db.json) — 10개 천간의 상징·성향·키워드·ILGAN_DETAIL (직업 추천 포함)
- [jiji_db.json](frontend/src/features/fortune/lib/jiji_db.json) — 12지지의 계절·상징·ILJI_DETAIL (일지별 상세 해석)
- [engine-data/sinsalMap.ts](frontend/src/features/fortune/lib/engine-data/sinsalMap.ts) — 천을귀인·문창귀인·역마살·도화살·화개살·겁살·재살 등 신살 매핑
- [engine-data/dailyReadings.ts](frontend/src/features/fortune/lib/engine-data/dailyReadings.ts) — 십성·12운성별 기본 문구
- [engine-data/categoryFortunes.ts](frontend/src/features/fortune/lib/engine-data/categoryFortunes.ts) — 5카테고리(재물·건강·연애·직장·학업) × 10십성 = 50 셀 기본 점수
...
프런트는 일간·일지·월지로 키를 만들어(`${ilgan}_${ilji}_${wolji}`) 원하는 셀만 꺼내 씀. [FortuneResult.tsx:427](frontend/src/features/fortune/components/FortuneResult.tsx).
...
| Lambda Function URL `2ranuwiguucfnrw7ks5jkjhami0zupuu.lambda-url.us-east-1.on.aws` | `/blog/admin/` 발행 | 블로그 글 S3 업로드 | ✅ [scripts/backend/lambda/blog-publish/](scripts/backend/lambda/blog-publish/) |
...
| GA4 (`googletagmanager.com`, `google-analytics.com`) | 전역 | 사용자 분석 | ✅ [shared/lib/GoogleAnalytics.tsx](frontend/src/shared/lib/GoogleAnalytics.tsx) |
| Microsoft Clarity (`clarity.ms`) | 전역 | UX 레코딩 | ✅ [shared/lib/ClarityAnalytics.tsx](frontend/src/shared/lib/ClarityAnalytics.tsx) |
...
[scripts/backend/lambda/blog-publish/handler.py](scripts/backend/lambda/blog-publish/handler.py). Function URL 방식 (API Gateway 없음).
...
**재배포 방법**:
```bash
cd scripts/backend/lambda/blog-publish
...
| CloudFront Function | 이름 조회 필요 | 글로벌 | `/xxx/` → `/xxx/index.html` 재작성 ([source](scripts/frontend/cloudfront/rewrite-subdir-index.js)) |
| CloudFront Response Headers Policy | `saju-sedaily-security-headers` | 글로벌 | HSTS/CSP/X-Frame-Options 등 ([source](scripts/frontend/cloudfront/apply-security-headers.sh)) |
...
# 또는
bash scripts/frontend/deploy.sh --skip-build  # 현재 out/ 그대로 배포
```

[scripts/frontend/deploy.sh](frontend/scripts/frontend/deploy.sh) 내부:
...
1. GitHub Actions 가 [scripts/backend/generate_blog_daily_zodiac.py](scripts/backend/generate_blog_daily_zodiac.py) 실행
2. 스크립트가 Bedrock Claude Sonnet 4 로 12별자리 운세 생성 (단일 호출, JSON 응답)
3. 로컬 `frontend/public/blog-content/posts/{YYYY-MM-DD}-daily-zodiac.json` 저장
4. `frontend/public/blog-content/index.json` 맨 앞에 entry 삽입 (slug dedupe)
...
- 로컬 재실행:
  ```bash
  python scripts/backend/generate_blog_daily_zodiac.py --date 2026-05-04
  ```
...
|---|---|---|---|
| [generate_saju_cache.py](scripts/backend/generate_saju_cache.py) | chongun + today (초기 단일 프로세스) | 수 시간 | $10~20 |
| [generate_parallel.py](scripts/backend/generate_parallel.py) | chongun 병렬 생성 (한국어) | 4~8시간 | $15~30 |
| [generate_parallel_en.py](scripts/backend/generate_parallel_en.py) | chongun 영어 버전 | 4~8시간 | $15~30 |
| [generate_today_parts.py](scripts/backend/generate_today_parts.py) | today-parts (파트별 리라이팅) | 1~2시간 | $3~8 |
| [generate_today_variants.py](scripts/backend/generate_today_variants.py) | 카테고리 설명 추가 variant (2개씩) | 1시간 | $3~5 |
| [generate_tone_variants.py](scripts/backend/generate_tone_variants.py) | 유리(favor) / 주의(caution) 톤 variant | 1~2시간 | $3~8 |
| [clean_today_cache.py](scripts/backend/clean_today_cache.py) | 반복 문구 정제 | 수 초 (로컬) | $0 |
...
pip install boto3
python scripts/backend/generate_parallel.py
```

### 11.4 진행률 재개

각 스크립트는 `scripts/backend/saju-cache-local/_progress*.txt` 로 진행률 기록. 중단되어도 재실행하면 완료된 조합은 스킵하고 이어서 진행.

### 11.5 결과물 경로

- 로컬: `scripts/backend/saju-cache-local/chongun/{일간}_{일지}_{월지}.json` 식으로 분산 저장
- S3: `s3://sedaily-mbti-frontend-dev/saju-cache/chongun/...` (역사적 이유, MBTI 버킷 사용)
- 프런트 번들: 빌드 타임에 합쳐서 `frontend/public/saju-cache/chongun.json` 단일 파일로. **합치는 스크립트는 현재 수동 또는 별도 절차로 관리 중** — 재생성 후 단일 JSON으로 병합하는 파이프라인은 문서화되지 않음 → 필요 시 재확인 필요
...
[scripts/backend/saju-cache-local/](scripts/backend/saju-cache-local/) 가 레포 용량 **94%**. 대략 2,200개 JSON. 원하면 `.gitignore` 로 제외하고 S3에만 보관할 수 있음. 단 재생성이 필요할 때 원본 확인이 어려워짐.
...
- **UI 라벨**: 100% KO/EN 토글. [LangContext](frontend/src/shared/lib/LangContext.tsx) + `t(ko, en)` 헬퍼
- **결과 서술 텍스트**: 한국어는 완성, 영어는 **진행 중 (2026-05-04 기준 ~53%)**. [scripts/backend/generate_parallel_en.py](scripts/backend/generate_parallel_en.py) 가 백그라운드로 생성 중
- **사주 용어 사전**: [sajuGlossary.ts](frontend/src/shared/constants/sajuGlossary.ts) — 천간/지지/오행/십성 등 KO↔EN 매핑
...
2. [FortuneResult.tsx](frontend/src/features/fortune/components/FortuneResult.tsx) 에서 `lang === 'en'` 일 때 `_en.json` 로드하도록 분기
...
- [GoogleAnalytics.tsx](frontend/src/shared/lib/GoogleAnalytics.tsx)
- Measurement ID: `process.env.NEXT_PUBLIC_GA_ID` (env 로 주입)
- opt-out: `localStorage.setItem('ga_disabled', 'true')`

### 13.2 Microsoft Clarity

- [ClarityAnalytics.tsx](frontend/src/shared/lib/ClarityAnalytics.tsx)
...
[trackEvent.ts](frontend/src/shared/lib/trackEvent.ts) 의 `trackEvent(name, params)` — GA4 로 custom event 발행. 주요 이벤트:
...
[shared/lib/jsonLd.tsx](frontend/src/shared/lib/jsonLd.tsx) 에서 schema.org 구조화 데이터 생성:
...
루트 [layout.tsx](frontend/src/app/layout.tsx) 에 전역 WebSite/Organization 주입. 각 `layout.tsx` 에 페이지별 FAQPage/BreadcrumbList.

### 14.2 Open Graph

각 페이지 `layout.tsx` 에 `openGraph`, `twitter` 메타 지정. 예시: [saju/layout.tsx](frontend/src/app/saju/layout.tsx).

### 14.3 sitemap.xml

[app/sitemap.ts](frontend/src/app/sitemap.ts) — 빌드 타임에 `sitemap.xml` 생성. 현재 8개 URL:
...
[public/robots.txt](frontend/public/robots.txt) — Google/Bing/Naver 외에 GPTBot·ClaudeBot·PerplexityBot·Applebot·Bytespider 등 AI 크롤러 13종 명시 허용.

[public/llms.txt](frontend/public/llms.txt) — LLM 답변 엔진용 사이트 맥락 제공 (사이트 개요, 핵심 개념, 커플 궁합 계산식, FAQ). **주요 기능 변경 시 llms.txt도 함께 업데이트** 필요.
...
[scripts/frontend/cloudfront/apply-security-headers.sh](scripts/frontend/cloudfront/apply-security-headers.sh) 참고. 외부 도메인 허용 리스트:
...
- 2020년 이전·2030년 이후 생년: 절기 데이터가 `@fullstackfamily/manseryeok` 범위 밖 → 근사치 폴백 ([engine.ts:getJeolgiApproxDates](frontend/src/features/fortune/lib/engine.ts#L307)). 대운수 1~2년 오차 가능
- 시간 모름: 시주만 제외, 나머지는 정상
- 경도 보정: 출생 도시별로 진태양시 보정. [CitySelect](frontend/src/features/fortune/components/CitySelect.tsx) 에서 관리
...
- 블로그 admin 비밀번호를 바꾸려면 **두 곳** 수정:
  1. [frontend/src/app/blog/admin/page.tsx](frontend/src/app/blog/admin/page.tsx) `ADMIN_PASS` 상수
...
| `scripts/backend/saju-cache-local/` 가 레포 용량 94% | git clone 느림, 파일 브라우징 느림 | 저 |
...
6. 이 문서와 [frontend/CLAUDE.md](frontend/CLAUDE.md) 한 번 더 읽기

### Q2. 사주 엔진에 새 해석을 추가하려면?

- 해석 "데이터": [engine-data/](frontend/src/features/fortune/lib/engine-data/) 에 테이블 추가 또는 기존 테이블 수정
- 해석 "로직": [engine.ts](frontend/src/features/fortune/lib/engine.ts) 에 순수 함수로 추가
- 해석 "UI": [FortuneResult.tsx](frontend/src/features/fortune/components/FortuneResult.tsx) 에 섹션 추가
...
2. [widgets/FeatureTabs.tsx](frontend/src/widgets/FeatureTabs.tsx) 의 `TABS` 배열에 추가 (원하면)
3. [app/sitemap.ts](frontend/src/app/sitemap.ts) 에 URL 추가
4. [public/llms.txt](frontend/public/llms.txt) 의 "주요 기능 페이지" 섹션에 추가
5. 빌드 + 배포
...
동시에 git 커밋도 남기고 싶으면 [scripts/backend/upload_blog_post.py](scripts/backend/upload_blog_post.py) 로 로컬 JSON 기반 업로드 가능.
...
- [features/fortune/components/WealthNewsSection.tsx](frontend/src/features/fortune/components/WealthNewsSection.tsx) 를 렌더하지 않거나 import 제거
...
```
577f866 docs(saju): README·CLAUDE·frontend/CLAUDE 사주 사이트 기준으로 재작성
