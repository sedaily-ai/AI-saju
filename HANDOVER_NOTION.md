...
- **파일**: [frontend/src/app/page.tsx](frontend/src/app/page.tsx)
...
- **파일**: [frontend/src/app/saju/page.tsx](frontend/src/app/saju/page.tsx) + `features/fortune/components/FortuneTab.tsx`
- **기능**:
  - 생년월일·시·성별·지역 입력
...
- **파일**: [frontend/src/app/chaeun/page.tsx](frontend/src/app/chaeun/page.tsx)
- **기능**:
...
- **파일**: [frontend/src/app/career/page.tsx](frontend/src/app/career/page.tsx)
- **기능**:
...
- **파일**: [frontend/src/app/compatibility/page.tsx](frontend/src/app/compatibility/page.tsx) + `features/ideal-match/`
- **기능**: 상대 없이 내 사주만으로 이상형 원국 역산 (일간·일지·오행·태어난 해·달)
- **백엔드 호출**: 없음

### 4-6. `/couple` — 커플 궁합

- **파일**: [frontend/src/app/couple/page.tsx](frontend/src/app/couple/page.tsx) + `features/couple-match/`
- **기능**: 두 사람 생년월일시 입력 → 0~10점 궁합 스코어 + 5축 근거 (일간·일지·오행보완·배우자성·연령차)
- **백엔드 호출**: 없음

### 4-7. `/news` — 경제뉴스 탐색

- **파일**: [frontend/src/app/news/page.tsx](frontend/src/app/news/page.tsx)
- **기능**: 사주 기반 키워드로 경제뉴스 검색
- **백엔드 호출**: `POST /api/search`

### 4-8. `/blog` — 블로그

- **파일**: [frontend/src/app/blog/page.tsx](frontend/src/app/blog/page.tsx)
...
- **파일**: [frontend/src/app/blog/admin/page.tsx](frontend/src/app/blog/admin/page.tsx)
...
2. `python scripts/backend/generate_blog_daily_zodiac.py` 실행
3. 스크립트가 Bedrock Claude Sonnet 4 호출 → 12별자리 운세 생성
4. 로컬 `frontend/public/blog-content/posts/{slug}.json` + `index.json` 저장
...
6. **주의**: git 에는 올라가지 않음. 싱크하려면 `git pull` 후 `scripts/backend/upload_blog_post.py` 사용

### 8-5. 사주 해석 캐시 재생성 (수동 배치)

1. 개발자 PC 에서 `python scripts/backend/generate_parallel.py` 실행
2. 스크립트가 Bedrock Claude Sonnet 4 를 1,440 조합 × 4 MBTI 톤 = 5,760 회 호출
3. 각 결과를 `scripts/backend/saju-cache-local/chongun/{일간}_{일지}_{월지}.json` 저장 + S3 업로드
4. (수동) 개별 JSON 을 `frontend/public/saju-cache/chongun.json` 단일 파일로 병합
5. 빌드 + 배포
...
`scripts/frontend/deploy.sh` 내부 동작:
...
```bash
bash scripts/frontend/cloudfront/apply-security-headers.sh
```
...
python scripts/backend/generate_parallel.py          # 한국어 1,440 조합 (~$30, ~8시간)
python scripts/backend/generate_parallel_en.py       # 영어 동일 (~$30, ~8시간)
python scripts/backend/generate_today_parts.py       # 오늘의 운세 파트 (~$8, 2시간)
```

진행 상황은 `scripts/backend/saju-cache-local/_progress*.txt` 에 기록되어 재개 가능.
...
| 목적 | 파일 |
|---|---|
| 랜딩 문구·CTA | `frontend/src/app/page.tsx` |
| 상단 탭 메뉴 | `frontend/src/widgets/FeatureTabs.tsx` |
| 사주 입력 폼 | `frontend/src/features/fortune/components/SajuInputPanel.tsx` |
| 사주 결과 전체 | `frontend/src/features/fortune/components/FortuneResult.tsx` |
| 재운 페이지 | `frontend/src/app/chaeun/page.tsx` |
| 커리어 페이지 | `frontend/src/app/career/page.tsx` |
| 이상형 결과 카드 | `frontend/src/features/ideal-match/components/IdealMatchSection.tsx` |
| 커플 궁합 결과 | `frontend/src/features/couple-match/components/CoupleMatchSection.tsx` |
| 블로그 목록·상세 | `frontend/src/app/blog/page.tsx` |
| 블로그 발행 UI | `frontend/src/app/blog/admin/page.tsx` |
| 전역 색상·폰트 | `frontend/src/app/globals.css` |
| 메타태그·OG·JSON-LD | 각 페이지의 `layout.tsx` |
| 사주 계산 로직 | `frontend/src/features/fortune/lib/engine.ts` |
| 재운·커리어 로직 | `frontend/src/features/fortune/lib/engine-chaeun.ts` |
| 궁합 점수 계산식 | `frontend/src/features/couple-match/lib/coupleEngine.ts` |
| 이상형 역산 로직 | `frontend/src/features/ideal-match/lib/matchEngine.ts` |
| AI 크롤러 정책 | `frontend/public/robots.txt` + `llms.txt` |
| CSP 보안 헤더 | `scripts/frontend/cloudfront/apply-security-headers.sh` |

### 새 페이지 추가하려면

1. `frontend/src/app/<route>/page.tsx` + `layout.tsx` 생성 (`layout.tsx` 는 기존 페이지 복사해서 메타·JSON-LD 수정)
2. 모든 페이지 컴포넌트는 `'use client'` 로 시작
3. `frontend/src/widgets/FeatureTabs.tsx` 의 `TABS` 배열에 탭 추가 (필요 시)
4. `frontend/src/app/sitemap.ts` 에 URL 추가
5. `frontend/public/llms.txt` 의 "주요 기능 페이지" 섹션에 추가 (AI 검색 노출용)
...
1. `scripts/frontend/cloudfront/apply-security-headers.sh` 열어서 해당 섹션에 도메인 추가
2. `bash scripts/frontend/cloudfront/apply-security-headers.sh` 실행
...
4. 또는 로컬:
   ```bash
   python scripts/backend/generate_blog_daily_zodiac.py --date 2026-XX-XX
   ```
...
| 원인 | 월 추가 비용 예상 |
|---|---|
| `scripts/backend/generate_parallel*.py` 실수로 전체 재실행 | $30~60 (Bedrock) |
...
**블로그 비밀번호 변경**: 프런트 (`frontend/src/app/blog/admin/page.tsx` 의 `ADMIN_PASS` 상수) + Lambda env 둘 다 수정.
...
| 사주 해석 캐시 생성 | ✅ (`scripts/backend/generate_*.py`) | 본인 재생성 |
...
- 로컬 JSON → `scripts/backend/upload_blog_post.py` 쓰면 S3 + git 모두 반영
...
- [ ] 이 문서 + `HANDOVER.md` + `frontend/CLAUDE.md` 정독
