"""
로컬 테스트 서버 — `npm run dev` 로 결정 카드 LLM 을 로컬에서 시험.
...
실행:
  ...
  # 끝나면 .env.local 의 그 줄을 지우면 다시 mock(무비용)으로 폴백.

환경변수: PORT(기본 8788), BEDROCK_MODEL_ID, MAX_TOKENS, AWS_REGION 은 handler.py 와 동일.
    ...
    def do_POST(self):  # noqa: N802
        try:
            ...
            if not payload.get("decisionText"):
                return self._send(400, {"error": "decisionText required"})
            text = handler._call_bedrock(handler._system(lang), handler._user(payload, lang))
...
if __name__ == "__main__":
    print(f"decision-card local server → http://localhost:{PORT}/  (model={handler.MODEL_ID})")
