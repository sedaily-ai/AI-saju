"""
오늘의 운세 파트별 리라이팅 배치 생성
- 십성 해석 (10개) × 4 MBTI = 40
- 12운성 해석 (12개) × 4 MBTI = 48
- 카테고리 설명 (5 × 10) × 4 MBTI = 200
총 288개 텍스트 → 하나의 JSON으로 병합
"""
import json
import boto3
import time
...
def make_prompt(kind: str, key: str, base_text: str, category: str = '') -> str:
    """리라이팅 요청 프롬프트"""
    if kind == 'ss':
        return f"""다음은 오늘의 운세에서 '{key}' 십성에 해당하는 사람에게 주는 해석 문장입니다.
이 내용을 당신의 성격과 말투에 맞게 리라이팅해 주세요.
원본의 핵심 메시지는 유지하되, 문체·어조·단어 선택은 당신의 성격에 맞게 자유롭게 바꾸세요.
3~5문장, 100~200자로 작성하세요. 마크다운 헤더 없이 자연스러운 문단으로만 써주세요.

[원본]
{base_text}"""
    elif kind == 'us':
        return f"""다음은 오늘의 운세에서 '{key}' 12운성 해석 문장입니다.
이 내용을 당신의 성격과 말투에 맞게 리라이팅해 주세요.
원본의 핵심 메시지는 유지하되, 문체·어조·단어 선택은 당신의 성격에 맞게 자유롭게 바꾸세요.
2~3문장, 60~120자로 작성하세요. 마크다운 헤더 없이 자연스러운 문단으로만 써주세요.
...
def process_task(task):
    """단일 태스크 처리"""
    kind, group, key, base_text, category = task
    client = get_bedrock_client()
    persona = MBTI_PERSONAS[group]
    prompt = make_prompt(kind, key, base_text, category)
    try:
        text = call_claude(client, persona, prompt)
        return (kind, group, key, category, text)
    except Exception as e:
        print(f'[ERROR] {kind}/{group}/{key}/{category}: {e}', flush=True)
        return (kind, group, key, category, None)
...
def build_tasks():
    ...
    for group in MBTI_PERSONAS:
        for key, base in SS_BASE.items():
            tasks.append(('ss', group, key, base, ''))
    # 12운성
    for group in MBTI_PERSONAS:
        for key, base in US_BASE.items():
            tasks.append(('us', group, key, base, ''))
    # 카테고리
    for group in MBTI_PERSONAS:
        for category, ss_map in CATEGORY_BASE.items():
            for key, base in ss_map.items():
                tasks.append(('category', group, key, base, category))
    return tasks
...
def main():
    ...
    if OUTPUT_FILE.exists():
        ...
        result = {'ss': {g: {} for g in MBTI_PERSONAS},
                  'us': {g: {} for g in MBTI_PERSONAS},
                  'category': {cat: {g: {} for g in MBTI_PERSONAS} for cat in CATEGORY_BASE}}

    tasks = build_tasks()
    # 이미 완료된 태스크 필터링
    remaining = []
    for t in tasks:
        kind, group, key, _, category = t
        if kind == 'category':
            if key in result.get('category', {}).get(category, {}).get(group, {}):
                continue
        else:
            if key in result.get(kind, {}).get(group, {}):
                continue
        remaining.append(t)

    total = len(tasks)
    completed = total - len(remaining)
    print(f'=== 오늘의 운세 파트 리라이팅: {total}개 (완료 {completed}, 남음 {len(remaining)}) ===', flush=True)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(process_task, t): t for t in remaining}
        for future in concurrent.futures.as_completed(futures):
            res = future.result()
            if res is None or res[4] is None:
                continue
            kind, group, key, category, text = res
            with save_lock:
                if kind == 'category':
                    result['category'][category][group][key] = text
                else:
                    result[kind][group][key] = text
                count[0] += 1
                # 10개마다 중간 저장
                if count[0] % 10 == 0 or count[0] == total:
                    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
                        json.dump(result, f, ensure_ascii=False, indent=2)
            print(f'[{count[0]}/{total}] {kind}/{group}/{category}/{key} ({len(text)}자)', flush=True)

    # 최종 저장
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    print(f'\n=== 완료: {count[0]}/{total}, 저장 위치: {OUTPUT_FILE} ===', flush=True)
